...
export function UpdateAvailableButton({ tvItem = false }: { tvItem?: boolean }) {
  ...
  const tvProps = tvItem
    ? ({ "data-tv-item": "", tabIndex: 0 } as const)
    : {};
  ...
  if (status.updateInProgress) {
    ...
    return (
      <button
        ...
        {...tvProps}
        className={cn(
          "flex h-9 max-w-[12rem] items-center gap-2 rounded-lg border border-primary/30 bg-primary/10 px-3 text-sm font-medium text-primary transition-colors hover:bg-primary/15",
          tvItem && "tv-focus-button outline-none",
        )}
      >
